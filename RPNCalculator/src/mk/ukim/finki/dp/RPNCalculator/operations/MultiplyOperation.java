...
import mk.ukim.finki.dp.RPNCalculator.mathoperations.MultiplyOperationImpl;
...
/**
 * Class for the operation of buttonMultiply.
 */
public class MultiplyOperation extends AbstractBasicOperation {
    public MultiplyOperation(TextView txtDisplayValue, RPNCalculatorStack stack, RPNCalculatorState state) {
    ...
    public void defaultStateOperation() {
        ...
        Double result = 0.0;
        MathOperation multiplyOperation = new MultiplyOperationImpl();
        if (!getStack().isStackEmpty()) {
            operand1 = getStack().popFromStack();
        }
        result = multiplyOperation.calculate(operand1, operand2);
    ...
    public void programStateOperation() {
        ProgramManipulationHandler.addProgramCharacter(Constants.ASTERISK);
        DisplayValueHelper.addCharToDisplay(getTxtDisplayValue(), Constants.ASTERISK);
    }
